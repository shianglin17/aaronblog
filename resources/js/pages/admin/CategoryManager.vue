          ...
          <div class="user-actions">
            <n-space :size="20" align="center">
              <div class="nav-tabs">
                ...
                <n-button class="nav-tab active" quaternary size="large" @click="$router.push('/admin/categories')">
                  分類管理
                </n-button>
              </div>
              <n-dropdown 
      ...
      <n-layout-content class="admin-content">
        <div class="content-wrapper">
          <div class="page-header">
            ...
            <n-space>
              <n-button type="primary" @click="openCreateModal">新增分類</n-button>
              <n-input
              ...
              />
              <n-button @click="fetchCategories">搜尋</n-button>
            </n-space>
          </div>
          <n-data-table
            :columns="columns"
            ...
            class="data-table"
            :empty-text="searchKeyword ? '沒有匹配的分類' : '尚未創建任何分類'"
          />
        </div>
        <!-- 新增/編輯分類 Modal -->
        <n-modal v-model:show="showEditModal" preset="dialog" :title="editMode ? '編輯分類' : '新增分類'">
...
import { authApi, categoryApi } from '../../api/index';
import type { Category, CreateCategoryParams, UpdateCategoryParams } from '../../types/category';
import type { User } from '../../types/auth';

// 表單型別與 CreateCategoryParams 保持一致，但增加 id 欄位用於編輯
interface CategoryForm extends CreateCategoryParams {
  id: number | null;
}
...
const editRules = {
  ...
  description: [{ required: true, message: '描述不能為空', trigger: 'blur' }]
};
